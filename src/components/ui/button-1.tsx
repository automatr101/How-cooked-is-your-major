  ...
  ({
    ...
    return (
      <button
        ...
        tabIndex={0}
        className={clsx(
          "flex justify-center items-center gap-0.5 duration-150 transition-all",
          sizes[+svgOnly][size],
          (disabled || loading) ? "bg-gray-100 text-gray-700 border border-gray-400 cursor-not-allowed opacity-50" : types[type],
          shapes[shape][size],
          shadow && "shadow-border-small border-none",
          fullWidth && "w-full",
          variant === "unstyled" ? "outline-none px-0 h-fit bg-transparent hover:bg-transparent text-gray-1000 p-0" : "focus:shadow-focus-ring focus:outline-0",
          className
        )}
        {...rest}
      >
