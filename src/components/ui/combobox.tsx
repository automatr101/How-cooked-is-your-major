...
const ComboboxInput = () => {
  ...
  const onBlur = () => {
    // Timeout to allow click on option to register before closing
    setTimeout(() => {
  ...
  return (
    <Input
      ...
      error={_errored}
      size={context?.size === "large" ? "large" : context?.size === "small" ? "small" : "medium"}
      ref={context?.inputRef}
      className={clsx(context?.errored && "text-red-900", "font-bold")}
    />
  );
};
...
const ComboboxList = ({ children, maxWidth, emptyMessage = "No results" }: {
  ...
  const context = useContext(ComboboxContext);
  const [position, setPosition] = useState<{ top?: number | string, bottom?: number | string }>();
  ...
  return (
    <Material
      ...
      type="menu"
      className={clsx(
        "absolute w-full z-[100] left-1/2 -translate-x-1/2 shadow-2xl border border-border mt-2",
    ...
    >
      <ul className="p-2 max-h-60 overflow-y-auto">
        {filteredChildren.length > 0 ? filteredChildren : (
          <li className={clsx(
            "flex justify-center items-center p-4 text-muted-foreground italic",
...
const ComboboxOption = ({ value, children }: ComboboxOptionProps) => {
  ...
  return (
    <li
      className={clsx(
        "flex justify-between items-center gap-2 cursor-pointer px-4 py-3 w-full rounded-xl hover:bg-primary/10 active:bg-primary/20 font-sans transition-all",
        context?.size === "large" ? "text-base" : "text-sm",
        value === context?.value ? "bg-primary/5 text-primary font-bold" : "text-foreground"
      )}
